# Domain 2 - Storage

This represents the sequence of lectures and the links to my notes to the lectures in this repository:

1. [Kinesis Streams Introduction (6:12)](Kinesis_Streams_Introduction.md)
2. [Kinesis Streams Core Concepts (17:24)](Kinesis_Streams_Core_Concepts.md)
3. [Kinesis Streams Emitting Data to AWS Services (12:57)](Kinesis_Streams_Emitting_Data_to_AWS_Services.md)
4. [Glacier (4:02)](Glacier.md)
5. [DynamoDB Introduction (11:43)](DynamoDB_Introduction.md)
6. [DynamoDB Partitions (11:16)](DynamoDB_Partitions.md)
7. [DynamoDB GSI/LSI (20:44)](DynamoDB_GSI_LSI.md)
8. [DynamoDB Streams and Replication (8:03)](DynamoDB_Streams_and_Replication.md)
9. [DynamoDB Performance Deep Dive Part 1 (15:29)](DynamoDB_Performance_Deep_Dive_Part_1.md)
10. [DynamoDB Performance Deep Dive Part 2 (10:35)](DynamoDB_Performance_Deep_Dive_Part_2.md)


## Resources

1.  [Kinesis Producer Library GitHub)](https://github.com/awslabs/amazon-kinesis-producer)
2.  [Kinesis Agent (GitHub)](https://github.com/awslabs/amazon-kinesis-agent)
3.  [Kinesis Client Library (GitHub)](https://github.com/awslabs/amazon-kinesis-client)
4.  [Implementing Efficient and Reliable Producers with the Amazon Kinesis Producer Library](https://aws.amazon.com/blogs/big-data/implementing-efficient-and-reliable-producers-with-the-amazon-kinesis-producer-library/)
5.  [Kinesis Connectors (GitHub)](https://github.com/awslabs/amazon-kinesis-connectors)
6.  [Scaling Writes on Amazon DynamoDB Tables with Global Secondary Indexes](https://aws.amazon.com/blogs/big-data/scaling-writes-on-amazon-dynamodb-tables-with-global-secondary-indexes/)
7.  [Improving Data Access with Secondary Indexes](http://docs.aws.amazon.com/amazondynamodb/latest/developerguide/SecondaryIndexes.html)
8.  [Cross-Region Replication](http://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Streams.CrossRegionRepl.html)

